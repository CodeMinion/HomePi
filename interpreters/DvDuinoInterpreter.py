...
# handle communications with the DvDuino
# server and the HomePi. The idea is 
# to have an app implementing the DvDuino
...
class DvDuinoInterpreter(object):
	...
	def connect(self):
		...
		try :
			...
			print 'SUCCESS: Connected to {0} - DVDuino'.format(self.server.getDeviceId())
			#time.sleep(10)
			# Start Keep Alive
	...
	def disconnect(self):
		...
		self.bmSocket.close();
		print 'Disconnected from {0} -  DVDuino'.format(self.server.getDeviceId())
